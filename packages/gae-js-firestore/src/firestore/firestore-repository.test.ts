...
describe("FirestoreRepository", () => {
  ...
  describe("collection group queries", () => {
    ...
    beforeEach(async () => {
      await repository.save([createItem("A"), createItem("B")]);
    ...
    describe("queryGroup", () => {
      describe("filters", () => {
        describe("filters specified by array", () => {
          it("filters by exact match", async () => {
            ...
            const results = await subRepositoryA.queryGroup({
              filters: [
                {
                  fieldPath: "name",
                  opStr: "==",
        ...
        describe("filters by class type", () => {
          it("filters by exact match", async () => {
            ...
            const results = await subRepositoryA.queryGroup({ filters: Filter.where("name", "==", "Test Item 234") });
      ...
      it("selects ids only when empty projection query", async () => {
        ...
        const results = await subRepositoryA.queryGroup({ select: [] });
      ...
      it("selects ids only when FIRESTORE_ID_FIELD projection query", async () => {
        ...
        const results = await subRepositoryA.queryGroup({ select: [FIRESTORE_ID_FIELD] });
      ...
      describe("limit and offset", () => {
        ...
        it("applies limit and offset", async () => {
          ...
          expect(results[0].id).toEqual("567");
          expect(results[1].id).toEqual("345");
        });
      });
      ...
      describe("ordering", () => {
        ...
        it("orders results ascending", async () => {
          ...
          expect(results.map(({ id }) => id)).toEqual(["123", "345", "234", "456", "567", "678"]);
        });
        ...
        it("orders results descending", async () => {
          ...
          expect(results.map(({ id }) => id)).toEqual(["678", "567", "456", "234", "345", "123"]);
        });
        ...
        it("orders results by nested field ascending", async () => {
          ...
          expect(results.map(({ id }) => id)).toEqual(["234", "123", "567", "345", "678", "456"]);
        });

        it("orders results by nested field descending", async () => {
          const results = await subRepositoryB.queryGroup({
            sort: {
              fieldPath: "nested.prop3",
              direction: "desc",
            },
          ...
          expect(results.map(({ id }) => id)).toEqual(["456", "678", "345", "567", "123", "234"]);
        });
        ...
        it("orders by multiple fields", async () => {
          ...
          expect(results.map(({ id }) => id)).toEqual(["567", "234", "123", "678", "456", "345"]);
        });
        ...
        it("orders results by id special key", async () => {
          ...
          expect(results.map(({ id }) => id)).toEqual(["123", "234", "567", "456", "678", "345"]);
        });
      });
      ...
      describe("cursors", () => {
        ...
        it("applies endBefore", async () => {
          ...
          expect(results.length).toBe(4);
          expect(results[2].id).toEqual("345");
        });
        ...
        it("applies multiple properties", async () => {
          ...
          expect(results.length).toBe(2);
          expect(results[0].id).toEqual("678");
          expect(results[1].id).toEqual("234");
        });
    ...
    describe("queryGroupForIds", () => {
      it("returns all matching id strings default sorted by full path when no filters applied", async () => {
        ...
        const ids = await subRepositoryA.queryGroupForIds();
    ...
    describe("countGroup", () => {
      describe("empty collection", () => {
        it("returns 0 when collection is empty", async () => {
          expect(await subRepositoryA.countGroup()).toBe(0);
          expect(await subRepositoryB.countGroup()).toBe(0);
        });
      });
      ...
      describe("with data", () => {
        ...
        it("counts all items in a collection by default", async () => {
          expect(await subRepositoryA.countGroup()).toBe(5);
        });

        it("counts all items matching filter", async () => {
          expect(
            await subRepositoryA.countGroup({
              filters: [{ fieldPath: "name", opStr: "==", value: "aaa" }],
            })
          ).toBe(3);
        });
